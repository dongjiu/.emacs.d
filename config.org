...
#+BEGIN_SRC emacs-lisp
  (defun z-upload-gpg ()
    "Encrypt and upload marked files."
    (interactive)
    (when (string= major-mode "dired-mode")
      (let ((files (dired-get-marked-files))
            ...
            (get-file-name
             (lambda (file)
               (file-name-nondirectory file))))
        (z-encrypt-files files (read-string "Key name: "))
        (dolist (file files)
          (z-upload-file (funcall get-gpg-file file)
                         (funcall get-file-id file)
                         (funcall get-file-name file))
          (z-add-file-tag (funcall get-file-id file) "GPG")))))
#+END_SRC
