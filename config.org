...
#+BEGIN_SRC emacs-lisp
  (defun z-encrypt-file (file key-name)
    ...
    (let ((context (epg-make-context)) (keys))
      (if (file-exists-p file)
          (progn (setq keys (epg-list-keys context key-name))
                 (if keys
                     (epg-encrypt-file context file keys nil)
...
Upload file:
#+BEGIN_SRC emacs-lisp
  (defun z-upload-file (file upload-dir upload-name)
    "Upload file to azure database."
    (let ((default-directory "~/.emacs.d/perl")
          (buffer "*UPLOAD FILE*"))
      (start-process "perl_upload_file" buffer
                     "perl" "azure_db.pl" "upload"
                     "--file" file
                     "--directory" upload-dir
                     "--upload_name" upload-name
                     "--password" (read-passwd "Database password: "))
      (display-buffer buffer)))
#+END_SRC

Pull file:
#+BEGIN_SRC emacs-lisp
  (defun z-pull-file (directory file-name output-file)
    "Pull file from azure database."
    (let ((default-directory "~/.emacs.d/perl")
          (buffer "*PULL FILE*"))
      (start-process "perl_pull_file" buffer
...
Delete file:
#+BEGIN_SRC emacs-lisp
  (defun z-delete-file (directory file-name)
    "Delete file from azure database."
    (let ((default-directory "~/.emacs.d/perl")
          (buffer "*DELETE FILE*"))
      (start-process "perl_delete_file" buffer
